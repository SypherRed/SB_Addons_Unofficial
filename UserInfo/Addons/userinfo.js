...
const config = yaml.load(
  fs.readFileSync("./Addons/Configs/userinfo.yml", "utf8")
);
const { Command } = require("../Structures/Addon");

module.exports.commands = [
  new Command({
    name: config.UserInfoCommand, // Name of command
    description: config.UserInfoCommandDesc, // Description of command
    options: [
      {
        type: "USER",
        name: "user",
    ...
    async run(interaction) {
        ...
        const UserNotExist = new Discord.MessageEmbed()
          ...
          .setDescription(
            `${supportbot.UserNotFound}\n\nTry Again:\`/${config.UserInfoCommand} <@!User_ID> or @User\``
          )
...
/**
 * @INFO
 * UserInfo command addon by Sypher#3415 | 321332200668790786
 * @INFO
